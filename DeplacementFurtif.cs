...
public class DeplacementFurtif : Deplacement {
    ...
    void Update()
    {
        ...
        if (readyToJump && GetJumps() > 0)
        {
            ...
            if (isAttached)
            {
                if ((propulseVector - originClic).magnitude < 10 && isWalking == false)
                {
                    walkOnWalls = StartCoroutine(WalkOnWalls());
                }
                if (t.verts > 2)
                {
                    if ((propulseVector - originClic).magnitude > 10 && isWalking == true)
                    {
        ...
        if (Input.GetButtonUp("Fire1") && GetJumps() > 0 && readyToJump && (propulseVector - originClic).magnitude > 10) //&& jumped == false)
        {
            if (isWalking == true)
            {
                StopCoroutine(walkOnWalls);
